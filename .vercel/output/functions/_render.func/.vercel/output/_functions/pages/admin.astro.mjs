import { c as createComponent, e as renderComponent, g as renderTemplate, m as maybeRenderHead, d as addAttribute } from '../chunks/astro/server_C3obWGp1.mjs';
import 'kleur/colors';
import { $ as $$AdminLayout } from '../chunks/AdminLayout_BD569xms.mjs';
...
</span> </div> </label> </div> <div class="flex gap-4 pt-4"> <button type="button" onclick="this.closest('dialog').close()" class="flex-1 px-6 py-3 rounded-xl border border-white/10 text-white/60 hover:bg-white/5 transition-colors">Cancel</button> <button type="submit" class="flex-1 btn-primary">Start Import</button> </div> </form> </div> </dialog> ` })} `;
